...
struct ImageOverlay
{
    std::uint16_t canvasFillValueR =
        0;  ///< R of RGBA pixel values used to fill canvas (which is not covered by any image)
    std::uint16_t canvasFillValueG =
        0;  ///< G of RGBA pixel values used to fill canvas (which is not covered by any image)
    std::uint16_t canvasFillValueB =
        0;  ///< B of RGBA pixel values used to fill canvas (which is not covered by any image)
    ...
    Vector<Offset>
        offsets;  ///< Offsets of images in the grid. Number and order must match to that of 'dimg' references.
};
