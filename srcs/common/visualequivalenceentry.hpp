...
public:
    ...
    ~VisualEquivalenceEntry() override = default;
    ...
    /** @brief Set timescale multiplier that specifies the timescale, as a multiplier to the media timescale of the
     * track.
     *  @param [in] multiplier Timescale multiplier  */
    void setTimescaleMultiplier(uint16_t multiplier);

    /** @brief Get  timescale multiplier that specifies the timescale, as a multiplier to the media timescale of the
     * track.
...
private:
    std::int16_t
        mTimeOffset;  ///< Difference of the time related to the image item(s) and the composition time of sample(s).
