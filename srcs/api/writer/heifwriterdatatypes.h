...
namespace HEIF
{
    typedef IdBase<std::uint32_t, class GroupTag> GroupId;
    typedef IdBase<std::uint32_t, class MediaDataTag> MediaDataId;
    ...
    struct HEIF_DLL_PUBLIC OutputConfig
    {
        ...
        * When parsing generated file whole file needs to be available for parsing to be possible. */
        ...
        /**
        * Brand four character code information stored to 'ftyp' box at the start of the file indicating content of the file.
        * If progressiveFile = false, then this information needs to be available when initialize() is called.
        * If progressiveFile = true, then further brands can be added between initialize() and finalize() with calls to 
    ...
    enum class MediaFormat
    {
        ...
        HEVC,   ///< Image(s) encoded with HEVC/H.265 encoder (Annex B in HEIF ISO/IEC 23008-12:2017(E) spec for details).
        JPEG,   ///< Image(s) encoded with JPEG encoder (Annex H in HEIF ISO/IEC 23008-12:2017(E) spec for details).
        EXIF,   ///< EXIF Metadata binary data (ExifDataBlock() as defined in Annex A.2.1 in HEIF ISO/IEC 23008-12:2017(E)).
    ...
    struct HEIF_DLL_PUBLIC SampleInfo
    {
        ...
        /** A sample that is predicted from other samples requires all its reference samples to be decoded prior to its decoding.
        *   Direct reference samples list 'refs' is a sample group that identifies all the direct reference samples for a sample. **/
    ...
    struct HEIF_DLL_PUBLIC EditList
    {
        ...
        double repetitions = 0.0;    ///< Edit list repetitions, a non-negative value. Only affects when looping is set true. Zero means infinite looping.
