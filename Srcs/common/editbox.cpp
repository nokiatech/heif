...
void EditBox::writeBox(BitStream& bitstr)
{
...
void EditBox::parseBox(BitStream& bitstr)
{
    ...
    while (bitstr.numBytesLeft() > 0)
    {
        ...
        if (boxType == "elst")
        {
            mEditListBox = std::make_shared<EditListBox>();
...
void EditListBox::addEntry(const EntryVersion0& entry)
{
    ...
    {
        throw std::runtime_error("Invalid attempt to add version0 EditListBox entries.");
    }
...
void EditListBox::addEntry(const EntryVersion1& entry)
{
    ...
    {
        throw std::runtime_error("Invalid attempt to add version1 EditList entries.");
    }
...
std::uint32_t EditListBox::numEntry() const
{
    ...
    switch(getVersion())
    {
        ...
        case 1:
            ...
            break;
        default:
            throw std::runtime_error("Not supported EditListBox entry version (only 0 and 1 are supported).");
    }
    return size;
}

void EditListBox::writeBox(BitStream& bitstr)
{
    ...
    if (mEntryVersion0.empty() == false)
    {
        ...
        for (const auto& entry : mEntryVersion0)
        {
            ...
            bitstr.write32Bits(entry.mMediaTime);
    ...
    else if (mEntryVersion1.empty() == false)
    {
        ...
        for (const auto& entry : mEntryVersion0)
        {
            ...
            bitstr.write64Bits(entry.mMediaTime);
            bitstr.write16Bits(entry.mMediaRateInteger);
            bitstr.write16Bits(entry.mMediaRateFraction);
        }
    }
...
void EditListBox::parseBox(BitStream& bitstr)
{
